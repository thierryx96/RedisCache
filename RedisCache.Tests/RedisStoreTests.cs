...
namespace RedisCache.Tests
{
    ...
    internal class RedisCacheStoreTests
    {
        ...
        public virtual void OneTimeSetUp()
        {
            ...
            _cacheType1 = new RedisCacheStore<TestType1Entity>(_connection, new JsonSerializer(), entity => entity.Id,
                null);
            _cacheType2 = new RedisCacheStore<TestType2Entity>(_connection, new JsonSerializer(), entity => entity.Id,
                null);
            _cacheType1WithExpiry = new RedisCacheStore<TestType1Entity>(_connection, new JsonSerializer(),
                entity => entity.Id, TimeSpan.FromSeconds(1));
        }

        private static readonly List<TestType1Entity> TestType1Entities =
            Enumerable.Range(0, 10).Select(i => new TestType1Entity {Name = $"testType1#{i}", Id = $"id#{i}"}).ToList();
